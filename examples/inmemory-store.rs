...
async fn main() -> KioResult<()> {
    ...
    let store = InMemoryStore::new(None, "test");
    ...
    let event_listener = move |state: EventParameters<_, _, _>| {
        ...
        async move {
            ...
            } = state
            {
                ...
                completed.fetch_add(1, std::sync::atomic::Ordering::Acquire);
            }
        }
    };
    ...
    let opts = WorkerOpts {
        //autorun: true,
        //concurrency: count as usize, // uncomment to use set concurrency
        ..Default::default()
    };
...
async fn process_callback(
    store: Arc<InMemoryStore<i32, i32, i32>>,
